...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.hg_frame = DoubleInputFrame(self, "Hermite Gauss Coefficients",
                                         "P Coefficient", "Q Coefficient")
        self.hg_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

        self.grating_frame = DoubleInputFrame(self, "Grating Parameters",
                                              "Wx Parameter", "Wy Parameter")
        self.grating_frame.grid(row=1,
                                column=0,
                                padx=10,
                                pady=10,
                                sticky="nsew")

        self.rect_frame = DoubleInputFrame(self, "Rectangle Size",
        ...
        self.effiency_result.grid(row=3,
                                  column=3,
                                  padx=10,
                                  pady=10,
                                  sticky="nsew")

        self.plot_button = customtkinter.CTkButton(self,
                                                   text="Plot Kinoform",
                                                   command=self.plot_graph)
        self.plot_button.grid(row=4, column=0, padx=10, pady=(20, 0))

        self.full_screen_button = customtkinter.CTkButton(
            self, text="Full Screen", command=self.full_screen)
        ...
        self.exit_button = customtkinter.CTkButton(self,
                                                   text="Exit",
                                                   command=self.on_exit)
        self.exit_button.grid(row=6, column=2, padx=5, pady=(20, 0))

        self.eff_button = customtkinter.CTkButton(
            self, text="Efficiency", command=self.efficiency_calcul)
        ...
        self.camera_canvas = customtkinter.CTkCanvas(self,
                                                     width=350,
                                                     height=350)
        self.camera_canvas.grid(row=0, column=5, rowspan=3)
        self.camera_canvas.configure(bg="black")

        self.camera_brightness = customtkinter.CTkSlider(
            self, from_=0, to=20, command=self.update_exposure)
